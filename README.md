...
| 파트 | 담당 | 주요 기여 |
|:---:|---|---|
| **A** | 가격·구매금액 / 모델링 파이프라인 통합 | 8-Step 전처리 구현, T/X-learner·DML 구축, Qini·ROI 평가 체계, 최종 Tier 리스트, 대시보드 |
| **B** | 연령×성별 세그먼트 | 분석 단위(`회원번호×index_date`) 설계, Y 기준 14일 확정(30일 ceiling effect 발견) |
| **C** | 계절·명절 상품 | `제철여부` 파생 설계, 원본 결측 8일 발견(Holdout 재설정의 계기), 위기서사 수치 정정 |
| **D** | 지역×배송조건 | 주소지 정규화·`region_key` 설계, 지역별 구독률 2.5배 격차 발견 |
