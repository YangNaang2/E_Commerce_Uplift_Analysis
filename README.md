...
팀 전체가 **가설별로 모듈을 나눠 병렬 분석하고 하나의 공통 파이프라인으로 통합**하는 방식으로 진행했습니다. 각자 전처리를 하면 결과를 비교할 수 없기 때문에, **분석 착수 전에 EDA 결과를 취합해 전처리 방식을 먼저 합의**한 후 진행했습니다.
...
친환경 신선식품 이커머스 기업의 사내 데이터마케팅팀 관점에서 착수한 프로젝트입니다.
