# 🛒 E-Commerce Uplift Analysis
### 인과추론 기반 프로모션 타겟팅 — "누구에게 쿠폰을 줘야 매출이 실제로 늘어나는가"
...
> **전체 고객에게 뿌리던 프로모션 예산을, "그 프로모션이 없었으면 사지 않았을 고객"에게만 집행하도록 재설계한 프로젝트.**
> 668,111행의 실거래 데이터에 **T-learner · X-learner · Double ML** 세 계열의 인과추론 모델을 적용해 개인별 처치효과(ITE)를 추정하고,
> 접촉 대상을 **8,276명 → 418명(-94.9%)** 으로 좁히면서 1인당 기대 Uplift는 **4.9–7.1배**로 끌어올린 타겟팅 리스트를 산출했습니다.
...
├── docs/                             # PRD·EDA 인사이트·모델링 통합결과·리더보드
...
5인 팀이 **가설별로 모듈을 나눠 병렬 분석하고 하나의 공통 파이프라인으로 통합**하는 방식으로 진행했습니다. 각자 전처리를 하면 결과를 비교할 수 없기 때문에, **분석 착수 전에 4파트의 EDA 결과를 취합해 전처리 규격을 먼저 합의**([`docs/전처리_통합_인사이트_ABCD.md`](docs/전처리_통합_인사이트_ABCD.md))한 것이 이 프로젝트 협업의 핵심입니다.
...
| [`docs/모델링_통합결과_H1_H4.md`](docs/모델링_통합결과_H1_H4.md) | 가장 상세한 방법론·수치 근거(가설별 검증 전문) |
| [`docs/리더보드_run10.md`](docs/리더보드_run10.md) | 전체 모델 리더보드 · 이중검증 · X-learner/DML 교차검증 |
