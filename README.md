...
친환경 신선식품 이커머스 기업(가상 케이스, FRESH.DATA)의 매출 하락을 데이터로 진단하고, **Uplift 모델링**으로 "프로모션을 줘야 실제로 반응하는 고객"만 골라내는 타겟팅 전략을 설계한 팀 프로젝트입니다. 청년 AI·BigData 아카데미 종합실습 과제로 진행했습니다.
...
| 파일 | 내용 |
|---|---|
| `대시보드_통합본.html` | 최종 통합 대시보드 (서버 불필요, 더블클릭으로 실행) |
| `전처리_리포트.html` (+발표대본) | 전처리 파이프라인 리포트 |
| `모델링_발표_초안.html` | Uplift 모델링 파트 발표 초안 |
| `현황_문제점_분석계획.html`/`.pdf` | 현황·문제점·분석계획 슬라이드 |
| `데이터_변수_브리핑.html`/`.pdf` | 변수 브리핑 |
| `이커머스_비즈니스모델캔버스.html`/`.png` | 비즈니스 모델 캔버스 |
...
`FRESH.DATA`는 과제용 가상 기업이며, 원본 데이터는 교육 목적으로 제공된 표본입니다. 실제 서비스나 실존 기업과 무관합니다.
