...
> 원본 데이터에 쿠폰·프로모션 노출 이력 컬럼이 없어, **구독 여부를 Treatment로 재정의**했습니다. 이 선택이 결론에 미치는 영향은 Double ML로 별도 검증했고, [기술 상세 문서 2-3절·4-5절](docs/기술_상세_문서.md#2-3-데이터-제약과-설계-판단-️)에 그 결과를 남겼습니다.
...
> 📎 Qini/AUUC·부트스트랩 신뢰구간·메타러너 간 순위상관, 그리고 이 수치들이 **어디까지 통계적으로 뒷받침되는지의 경계**는 [기술 상세 문서 3-3절·4-5절](docs/기술_상세_문서.md#3-3-평가-지표--인과추론-특화-지표-적용)에 정리돼 있습니다.
...
서버 없이 결과만 보려면 [`reports/대시보드_통합본.html`](reports/대시보드_통합본.html)을 열면 됩니다.
...
│   ├── PRD.md                            # 프로젝트 정의서
│   ├── 모델링_통합결과_H1_H4.md           # 가설별 검증 전문
│   ├── 리더보드_run10.md                  # 전체 모델 리더보드
│   └── 전처리_통합_인사이트_ABCD.md        # 4파트 EDA 취합 → 공통 전처리 합의문
...
팀 전체가 **가설별로 모듈을 나눠 병렬 분석하고 하나의 공통 파이프라인으로 통합**하는 방식으로 진행했습니다. 각자 전처리를 하면 결과를 비교할 수 없기 때문에, **분석 착수 전에 EDA 결과를 취합해 전처리 규격을 먼저 합의**([`docs/전처리_통합_인사이트_ABCD.md`](docs/전처리_통합_인사이트_ABCD.md))한 것이 협업의 핵심이었습니다.

| 팀원 | 담당 | 가설 | 주요 기여 |
|:---:|---|:---:|---|
| **양진우** | 모델 A — 가격·구매금액 | H1 | T-learner 구축, 모델링 파이프라인 통합, 최종 Tier 리스트 산출 |
| **임성민** | 모델 B — 연령×성별 세그먼트 | H4 | 분석 단위(`회원번호×index_date`) 설계, Y 기준 14일 확정(30일 ceiling effect 발견) |
| **박승형** | 모델 C — 계절·명절 상품 | H3 | `제철여부` 파생 설계, 원본 결측 8일 발견(Holdout 재설정의 계기), 위기서사 수치 정정 |
| **박지훈** | 모델 D — 지역×배송조건 / 데이터 전처리 | H2 | 주소지 정규화·`region_key` 설계, 지역별 구독률 2.5배 격차 발견 |
| **이선희** | 데이터 전처리 / 대시보드 | — | 전처리 규격 설계·검증, FastAPI + Streamlit 대시보드 구현 및 제작 |
| **강인수** | 비즈니스 케이스 스터디 / 발표자료 | — | 네이버페이·Wayfair 적용 사례 조사, 최종 발표 덱 제작 |
...
전체 파이프라인 재실행 절차는 [기술 상세 문서 7절](docs/기술_상세_문서.md#-7-재현-방법-getting-started)을 참고하세요.
