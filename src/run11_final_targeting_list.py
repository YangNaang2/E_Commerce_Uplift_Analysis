...
**설계 근거**: 기존 버전은 A/B/C/D "4파트 만장일치"(run09) 기반이었으나, B/C/D 개인별 점수가
실제 팀원 코드가 아니라 동일 방법론으로 재구성한 버전이라 만장일치 인원이 인위적으로 부풀려지는
문제가 확인됨(방법론이 사실상 같은 4개 변형을 비교한 것이라 서로 과도하게 닮아있음). 발표에서
캐비엇 없이 쓸 수 있는 숫자를 만들기 위해, **100% 실제 데이터로 학습된 A 본인의 두 모델(run01b
로지스틱 / run08b RandomForest, 둘 다 10/26 Holdout 기준)의 교집합**으로 타겟팅 로직을 짠다.
